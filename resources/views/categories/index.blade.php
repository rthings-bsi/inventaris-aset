...
<div class="mb-5 relative z-30 flex items-center justify-between gap-4 w-full">
    <form method="GET" action="{{ route('categories.index') }}" class="w-full max-w-sm">
        <div class="relative flex items-center px-4 py-2 bg-white rounded-2xl shadow-sm border border-gray-100 focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-50 transition-all">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari category..." class="w-full pl-3 pr-2 border-none focus:ring-0 text-[14px] bg-transparent outline-none text-gray-800 font-medium">
            @if(request('search'))
                <a href="{{ route('categories.index') }}" class="text-gray-400 hover:text-red-500 transition-colors"><i class="fas fa-times text-sm"></i></a>
            @endif
        </div>
    </form>
    
    <div class="flex items-center justify-end flex-shrink-0">
        <a href="{{ route('categories.create') }}" class="group relative inline-flex items-center justify-center px-5 py-2.5 text-[13px] font-black text-white transition-all duration-300 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl shadow-[0_5px_15px_rgba(79,70,229,0.35)] hover:shadow-[0_10px_25px_rgba(79,70,229,0.45)] hover:-translate-y-0.5 whitespace-nowrap border border-transparent">
            <i class="fas fa-plus mr-2 group-hover:rotate-90 transition-transform duration-300"></i> Add Category
        </a>
    </div>
</div>
...
<div class="glass-card rounded-[2rem] relative border-t-2 border-l-2 border-white/80 mb-8 shadow-sm">
    ...
    <div class="hidden md:block overflow-x-auto px-4 pt-1 pb-4 md:px-6 md:pt-2 md:pb-6">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr>
                    <th class="px-6 py-4 text-[10px] font-black text-indigo-400 uppercase tracking-widest border-b-2 border-dashed border-indigo-100">Nama Category</th>
            ...
            <tbody class="space-y-2">
                ...
                <tr class="group hover:bg-white/80 transition-all duration-300 rounded-2xl border-b border-gray-100 hover:shadow-[0_2px_15px_rgba(79,70,229,0.05)] relative">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <h4 class="text-[15px] font-black text-gray-800">{{ $category->category_name }}</h4>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-[13px] font-medium text-gray-600 truncate max-w-xs block">{{ $category->description ?: '-' }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        <div class="flex items-center justify-end gap-2 transition-all duration-300">
                            <a href="{{ route('categories.edit', $category) }}" class="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-50 border border-gray-100 text-purple-500 hover:bg-purple-500 hover:text-white hover:border-purple-500 transition-all hover:scale-110 hover:rotate-6 shadow-sm" title="Modifikasi Entri">
                                <i class="fas fa-edit text-sm"></i>
                            </a>
                            <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete(this, 'Category')" class="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-50 border border-gray-100 text-red-500 hover:bg-red-500 hover:text-white hover:border-red-500 transition-all hover:scale-110 hover:rotate-12 shadow-sm" title="Hapus Permanen">
        ...
        <div class="bg-white rounded-[1.25rem] p-4 border border-indigo-50 shadow-[0_5px_15px_-5px_rgba(79,70,229,0.08)] flex flex-col gap-3 relative overflow-hidden group">
            <div class="flex gap-3 items-start justify-between border-b border-gray-100 pb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="text-[14px] font-black text-gray-800 leading-tight">{{ $category->category_name }}</h4>
                    </div>
                </div>
                <!-- Actions -->
                <div class="flex gap-1.5 flex-shrink-0">
                    <a href="{{ route('categories.edit', $category) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-purple-50 text-purple-500 hover:bg-purple-500 hover:text-white transition-all shadow-sm">
            ...
            <div class="pt-1">
                ...
                <span class="font-bold text-gray-700 text-[12px] line-clamp-3">{{ $category->description ?: '-' }}</span>
            </div>
            ...
            <div class="mt-2 text-right border-t border-gray-100 pt-3">
                <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="confirmDelete(this, 'Category')" class="text-[10px] uppercase tracking-wider font-bold text-red-400 hover:text-red-600 flex items-center justify-end w-full gap-1.5 transition-colors">
    ...
    <div class="flex flex-col items-center justify-center py-24 text-center relative z-10 px-4">
        ...
        <p class="text-[15px] font-semibold text-gray-500 max-w-md mx-auto mb-8 leading-relaxed">Sistem belum memiliki entri category yang dicari.</p>
        <a href="{{ route('categories.create') }}" class="px-8 py-3.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-black transition-all group border border-transparent">
            <i class="fas fa-plus mr-3"></i> Tambah Category
        </a>
    </div>
    @endif
</div>
