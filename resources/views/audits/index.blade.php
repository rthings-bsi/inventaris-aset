...
<!-- Page Header -->
<div class="relative glass-header rounded-[2rem] p-6 md:p-10 mb-8 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-2 border-white group">
    ...
    <div class="relative z-10 w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight flex items-center gap-3">
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-purple-700">Audit Data Aset</span> 📑
            </h1>
            <p class="text-gray-600 mt-2 font-semibold text-lg max-w-xl">Lakukan rekonsiliasi stok fisik melalui pemindaian barcode digital.</p>
        </div>
        <button onclick="document.getElementById('new-audit-modal').classList.remove('hidden')" class="group relative inline-flex items-center justify-center px-6 py-3.5 text-[14px] font-black text-white transition-all duration-300 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-[0_10px_25px_rgba(79,70,229,0.35)] hover:shadow-[0_15px_35px_rgba(79,70,229,0.45)] hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-indigo-300 border border-transparent">
...
<!-- Main Content Area -->
<div class="glass-card rounded-[2.5rem] relative border-t-2 border-l-2 border-white/80 mb-8 shadow-sm overflow-hidden min-h-[500px]">
    <!-- Animated background effects inside the card -->
    <div class="absolute inset-0 overflow-hidden pointer-events-none -z-10 opacity-40">
        <div class="absolute top-0 right-0 w-64 h-64 bg-indigo-100 rounded-full filter blur-3xl animate-pulse"></div>
        <div class="absolute bottom-0 left-0 w-64 h-64 bg-purple-100 rounded-full filter blur-3xl animate-float"></div>
    </div>
            ...
            <table class="w-full text-left border-separate border-spacing-y-4">
                <thead>
                    <tr class="text-[10px] font-black uppercase text-gray-400 tracking-widest">
                        <th class="px-6 py-2">Informasi Sesi</th>
                ...
                <tbody>
                    ...
                    <tr class="group hover:scale-[1.01] transition-all duration-300">
                        <td class="bg-white/80 rounded-l-[1.5rem] px-6 py-5 border-y border-l border-indigo-50/50 shadow-sm first:pl-8">
                        ...
                        <td class="bg-white/80 px-6 py-5 border-y border-indigo-50/50 shadow-sm">
                        ...
                        <td class="bg-white/80 px-6 py-5 border-y border-indigo-50/50 shadow-sm text-center">
                        ...
                        <td class="bg-white/80 rounded-r-[1.5rem] px-6 py-5 border-y border-r border-indigo-50/50 shadow-sm pr-8 text-right">
                            <div class="flex items-center justify-end gap-2.5">
                                @if($audit->status == 'open')
                                    <a href="{{ route('audits.show', $audit) }}" class="w-10 h-10 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-200 flex items-center justify-center hover:scale-110 hover:-rotate-3 transition-all" title="Lanjut Scanning">
                                        <i class="fas fa-bolt"></i>
                                    </a>
                                @else
                                    <a href="{{ route('audits.report', $audit) }}" class="w-10 h-10 bg-white border border-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center hover:bg-indigo-50 hover:scale-110 transition-all shadow-sm" title="Lihat Laporan">
                                        <i class="fas fa-file-alt"></i>
                                    </a>
                                @endif
                                <form action="{{ route('audits.destroy', $audit) }}" method="POST" class="inline delete-form">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="handleDelete(this)" class="w-10 h-10 bg-white border border-rose-100 text-rose-500 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-700 hover:scale-110 transition-all shadow-sm" title="Hapus Sesi">
                    ...
                    <tr>
                        <td colspan="5" class="py-24 text-center">
                            <div class="inline-flex flex-col items-center">
                                <div class="w-24 h-24 bg-indigo-50 text-indigo-200 rounded-[2rem] flex items-center justify-center mb-6 shadow-inner animate-pulse">
                                    <i class="fas fa-search text-4xl"></i>
                                </div>
                                <h4 class="text-xl font-black text-gray-800 mb-1">Belum Ada Sesi Audit</h4>
                                <p class="text-sm font-bold text-gray-400">Silakan buat sesi audit baru untuk mulai memindai aset.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
