...
@section('content')
<x-page-header 
    title="Ubah Informasi Aset" 
    subtitle="Anda sedang memperbarui data {!! $asset->asset_name !!}. Pastikan validitas data." 
    emoji="✏️"
>
    <x-slot name="actions">
...
<div class="glass-card rounded-[2rem] p-8 md:p-10 mb-10 relative overflow-hidden group/form border-t-2 border-l-2 border-white/80">
    ...
    <form action="{{ route('assets.update', $asset) }}" method="POST" enctype="multipart/form-data" class="relative z-10">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="relative">
                    ...
                    <select name="id_categories" required
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id_categories }}" {{ old('id_categories', $asset->id_categories) == $category->id_categories ? 'selected' : '' }}>{{ $category->category_name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="group/input relative pb-1">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="acquisition_cost" value="{{ old('acquisition_cost', $asset->acquisition_cost) }}" 
            ...
            <div class="group/input pt-2">
                ...
                <div class="relative">
                    <input type="date" name="acquisition_date" value="{{ old('acquisition_date', $asset->acquisition_date->format('Y-m-d')) }}" 
            ...
            <div class="group/input pt-2">
                ...
                <div class="relative">
                    ...
                    <select name="condition" required
                        ...
                        <option value="Baik Sekali" {{ old('condition', $asset->condition) == 'Baik Sekali' ? 'selected' : '' }}>Grade A - Masih Baru / Sangat Mulus</option>
                        <option value="Baik" {{ old('condition', $asset->condition) == 'Baik' ? 'selected' : '' }}>Grade B - Condition Baik Kelayakan 100%</option>
                        <option value="Cukup" {{ old('condition', $asset->condition) == 'Cukup' ? 'selected' : '' }}>Grade C - Layak Pakai dengan Defect Minor</option>
                        <option value="Kurang Baik" {{ old('condition', $asset->condition) == 'Kurang Baik' ? 'selected' : '' }}>Grade D - Kurang Baik / Perlu Inspeksi</option>
                        <option value="Rusak" {{ old('condition', $asset->condition) == 'Rusak' ? 'selected' : '' }}>Grade E - Rusak Parah / Unserviceable</option>
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-5 text-indigo-400">
                        <i class="fas fa-chevron-down text-sm"></i>
                    </div>
                ...
                <div class="relative">
                    ...
                    <select name="id_locations" required
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id_locations }}" {{ old('id_locations', $asset->id_locations) == $location->id_locations ? 'selected' : '' }}>{{ $location->location_name }}</option>
                        @endforeach
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-5 text-indigo-400">
                        <i class="fas fa-chevron-down text-sm"></i>
            ...
            <div class="group/input md:col-span-1">
                ...
                <div class="relative">
                    ...
                    <select name="status" required
                        ...
                        <option value="maintenance" {{ old('status', $asset->status) == 'maintenance' ? 'selected' : '' }}>Maintenance - Dalam Perbaikan / Servis</option>
                        <option value="broken" {{ old('status', $asset->status) == 'broken' ? 'selected' : '' }}>Rusak - Tidak Dapat Diberdayakan</option>
                        <option value="disposed" {{ old('status', $asset->status) == 'disposed' ? 'selected' : '' }}>Dihapuskan - Proses Penghancuran / Lelang</option>
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-5 text-indigo-400">
                        <i class="fas fa-chevron-down text-sm"></i>
                    </div>
                ...
                <div class="relative">
                    ...
                    <select name="id_users"
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id_users }}" {{ old('id_users', $asset->id_users) == $user->id_users ? 'selected' : '' }}>{{ $user->name }} - {{ $user->email }}</option>
                        @endforeach
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-5 text-indigo-400">
                        <i class="fas fa-chevron-down text-sm"></i>
            ...
            <!-- Photo -->
            <div class="group/input md:col-span-1">
                <label class="block text-xs font-black text-indigo-900 mb-2 uppercase tracking-widest group-hover/input:text-indigo-600 transition-colors">Pembaruan Dokumen / Photo Benda</label>
                <div class="border-2 border-dashed border-indigo-200 bg-indigo-50/30 rounded-3xl p-6 text-center hover:bg-indigo-50 hover:border-indigo-400 transition-all cursor-pointer relative group/upload overflow-hidden" onclick="document.getElementById('photo-input').click()">
                    <input type="file" name="photo" id="photo-input" accept="image/*"
                           class="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10" onchange="previewText(this)">
                    
                    @if($asset->photo)
                        <div class="mb-4 flex flex-col items-center">
                            <div class="relative w-20 h-20 rounded-2xl overflow-hidden shadow-sm group-hover/upload:rotate-3 transition-transform border-4 border-white">
                                <img src="{{ asset('storage/' . $asset->photo) }}" alt="{{ $asset->asset_name }}" class="w-full h-full object-cover">
                                <div class="absolute inset-0 bg-indigo-900/30 flex items-center justify-center opacity-0 group-hover/upload:opacity-100 transition-opacity">
                                    <i class="fas fa-sync-alt text-white animate-spin-slow"></i>
                                </div>
                            </div>
                            <span class="bg-indigo-100 text-indigo-700 font-bold text-[10px] uppercase tracking-wider px-3 py-1 rounded-full mt-3 inline-block shadow-sm">File Lama Tersimpan</span>
                        </div>
                        <div class="flex flex-col items-center gap-2 mt-2 text-indigo-400 pointer-events-none transition-transform group-hover/upload:-translate-y-1">
                            <span class="text-[14px] font-black text-indigo-600" id="file-chosen">Timpa Berkas dengan File Baru...</span>
                        </div>
                    @else
                        <div class="flex flex-col items-center gap-3 mt-2 text-indigo-400 pointer-events-none transition-transform group-hover/upload:-translate-y-2">
                            ...
                            <span class="text-[15px] font-black text-indigo-600" id="file-chosen">Pilih Direktori Berkas Baru...</span>
                        </div>
                    @endif

                    <p class="text-[10px] font-black tracking-widest uppercase text-gray-400 mt-4 pointer-events-none relative z-20">Ekstensi Maks: JPG/PNG • 2MB</p>
                </div>
                @error('photo')
                    <p class="text-red-500 text-xs font-bold mt-2 animate-bounce-sm px-1"><i class="fas fa-info-circle mr-1"></i> {{ $message }}</p>
                @enderror
            ...
            <script>
                function previewText(input) {
                    ...
                    if (input.files && input.files[0]) {
                        fileChosen.textContent = 'Berkas Siap Ditimpa: ' + input.files[0].name;
                        fileChosen.classList.remove('text-indigo-600');
                        fileChosen.classList.add('text-pink-500');
                    } else {
                        fileChosen.textContent = '{{ $asset->photo ? 'Timpa Berkas dengan File Baru...' : 'Pilih Direktori Berkas Baru...' }}';
            ...
            <div class="group/input md:col-span-1">
                ...
                <div class="relative h-full">
                    <textarea name="description" rows="6"
                              ...
                              placeholder="Dokumentasikan spesifikasi, riwayat garansi atau info unik lainnya...">{{ old('description', $asset->description) }}</textarea>
                </div>
                @error('description')
                    <p class="text-red-500 text-xs font-bold mt-2 animate-bounce-sm px-1"><i class="fas fa-info-circle mr-1"></i> {{ $message }}</p>
                @enderror
        ...
        <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-4 mt-10 pt-8 border-t-2 border-dashed border-indigo-100">
            ...
            <button type="submit" class="w-full sm:w-auto px-10 py-3.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-black shadow-[0_10px_20px_-10px_rgba(79,70,229,0.5)] hover:shadow-[0_15px_30px_-10px_rgba(79,70,229,0.7)] hover:-translate-y-1 transition-all group border border-transparent">
                <span class="flex items-center justify-center">
                    Simpan Pembaruan <i class="fas fa-save ml-3 group-hover:scale-125 transition-transform"></i>
                </span>
            </button>
        </div>
    </form>
